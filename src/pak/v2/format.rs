...
impl VPKHeaderV2 {
    pub fn from(file: &mut VPKFile) -> Self {
        ...
        let file_data_section_size = file
            .read_u32()
            .expect("Could not read header file data section size from file");
        let archive_md5_section_size = file
            .read_u32()
            .expect("Could not read header archive MD5 section size from file");
        let other_md5_section_size = file
            .read_u32()
            .expect("Could not read header other MD5 section size from file");
        let signature_section_size = file
            .read_u32()
            .expect("Could not read header signature section size from file");
        ...
        assert_eq!(
            archive_md5_section_size % 28,
            0,
            "VPK header archive MD5 section size should be a multiple of 28"
        );
        assert_eq!(
            other_md5_section_size, 48,
            "VPK header other MD5 section size should be 48"
        );
        assert!(
            signature_section_size == 0 || signature_section_size == 296,
            "VPK header signature section size should be 0 or 296"
        );
...
impl VPKVersion2 {
    ...
    pub fn from(file: &mut VPKFile) -> Self {
        ...
        let file_data = file
            ...
            .expect("Failed reading file data section");
        ...
        while archive_md5_section_entries.len() < (header.archive_md5_section_size / 28) as _ {
            archive_md5_section_entries.push(VPKArchiveMD5SectionEntry {
                archive_index: file
                    .read_u32()
                    .expect("Failed reading archive MD5 section archive index"),
                starting_offset: file
                    .read_u32()
                    .expect("Failed reading archive MD5 section starting offset"),
                count: file
                    .read_u32()
                    .expect("Failed reading archive MD5 section count"),
                md5_checksum: file
                    .read_bytes(16)
                    .expect("Failed reading archive MD5 section count"),
            });
        ...
        let other_md5_section = VPKOtherMD5Section {
            tree_checksum: file
                .read_bytes(16)
                .expect("Failed reading other MD5 section tree checksum"),
            archive_md5_section_checksum: file
                .read_bytes(16)
                .expect("Failed reading other MD5 section archive MD5 section checksum"),
            unknown: file
                .read_bytes(16)
                .expect("Failed reading other MD5 section unknown"),
        };
